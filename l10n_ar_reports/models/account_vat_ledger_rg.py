...
class AccountVatLedger(models.Model):
    ...
    def _get_pos_and_invoice_invoice_number(self, invoice):
        res = invoice._l10n_ar_get_document_number_parts(
            invoice.l10n_latam_document_number, invoice.\
    ...
    def _get_REGINFO_CV_CBTE(self, alicuotas):
        ...
        for inv in invoices:
            ...
            perc_imp_nacionales_amount = amounts['profits_perc_amount'] + \
                amounts['other_perc_amount']

            if vat_exempt_base_amount:
                # operacion con zona franca
                if inv.partner_id.l10n_ar_afip_responsibility_type_id.\
                    code == '10':
                    codigo_operacion = 'Z'
                # expo al exterior
            ...
            if self.type == 'sale':
                ...
                # En el resto de los casos se consignará el dato registrado en 
                # el campo 4
            ...
            if self.type == 'sale':
                row += [
                    ...
                    # la figura no categorizado / responsable no inscripto 
                    # no se usa más
                    ...
                    self.format_amount(
                        perc_imp_nacionales_amount + vat_perc_amount),
                ]
            else:
                row += [
                    ...
                    # Campo 13: Importe de percepciones o pagos a cuenta otros 
                    # impuestos nacionales
            ...
            if self.type == 'sale':
                ...
                row += [
                    ...
                    # consigna '033', '058', '059', '060' ó
                    # '063'. Si para éstos comprobantes no interviene un 
                    # tercero en la operación, se consignará la
                    # C.U.I.T. del informante. Para el resto de los 
                    # comprobantes se completará con ceros
                    self.format_amount(
                        liquido_type and inv.company_id.partner_id.\
                            ensure_vat() or 0, padding=11),

                    # Campo 24: Denominación Emisor / Corredor
                    (liquido_type and inv.company_id.name or '').ljust(
                        30, ' ')[:30],
    ...
    def _get_tax_row(self, invoice, base, code, tax_amount, impo=False):
        self.ensure_one()
        inv = invoice
        invoice_number, pos_number = self._get_pos_and_invoice_invoice_number(
            inv)
        doc_code, doc_number = self._get_partner_document_code_and_number(
    ...
    def _get_REGINFO_CV_ALICUOTAS(self, impo=False):
        ...
        # URL ( http://contadoresenred.com/regimen-de-informacion-de-compras-y-
        # ventas-rg-3685-como-cargar-la-informacion/ )
        # empezamos a contar los codigos 1 (no gravado) y 2 (exento) si no hay 
        # alicuotas, sumamos una de esta con 0, 0, 0 en detalle usamos mapped 
        # por si hay afip codes duplicados (ej. manual y auto)
        if impo:
        ...
        for inv in invoices:
            ...
            if not vat_taxes and inv.l10n_latam_document_type_id.\
