...
def set_tax_included(cr, registry):
    ...
    env = api.Environment(cr, SUPERUSER_ID, {})
    # if this option was never configured we set it as tax_included. This is 
    # useful for ecommerce
    # where in argentina normally ecommerce are b2c. account_ux and sale_ux 
    # areadly modify the backend
    ...
    if not env.ref('account.show_line_subtotals_tax_selection', False):
        ...
        for groupId in ['base.group_portal', 'base.group_user', 
            'base.group_public']:
            group = env.ref(groupId, False)
            if not group:
                continue
            group.write({'implied_ids': [
                        (3, env.ref('account.group_show_line_subtotals_tax_excluded').id)]})
            env.ref('account.group_show_line_subtotals_tax_excluded').write(
                {'users': [(5,)]})

            group.write({'implied_ids': [
                        (4, env.ref('account.group_show_line_subtotals_tax_included').id)]})
