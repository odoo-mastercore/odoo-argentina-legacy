        ...
        for rec in self:
            ...
            if rec.afip_auth_code:
                invoice = {
                    ...
                    'cuit': int(
                        str(rec.company_id.partner_id.vat).replace('-','')),
        ...
        for inv in self:
            ...
            #     )
                ...
                ws.CrearFactura(
                    ...
                    id_impositivo, moneda_id, round(float(moneda_ctz), 2), obs_comerciales,
                ...
                imp_internos = amounts['intern_tax_amount'] + \
                    amounts['other_taxes_amount']
                imp_perc = amounts['vat_perc_amount'] + \
                ...
                if afip_ws == 'wsfex':
                    ...
                    ws.AgregarCmpAsoc(
                        ...
                        afip_ws != 'wsmtxca' and CbteAsoc.invoice_date.strftime(
                            '%Y%m%d') or CbteAsoc.invoice_date.strftime(
                                '%Y-%m-%d')
                    )
